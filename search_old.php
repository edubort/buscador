...
/** INCLUIDO POR EDUARDO BORTOLOTTI - 03/01/2015 **
if(empty($_SESSION['ssPrimeiroAcesso']) or intval($_GET['inicio']) == 1) {
  $strCssSearchResult = "searchBox"; 
  //require_once("$template_dir/$template/search_form.html");
  $_SESSION['ssPrimeiroAcesso'] = 1;
} else {
  
  $strCssSearchResult = "searchBoxResult";
  //require_once("$template_dir/$template/search_form.html");
}
*/
...
switch ($search) {
  case 1:
    ...
    require("templates/$strTemplate/search_results.html");
  break;
  default:
    if ($show_categories) {
